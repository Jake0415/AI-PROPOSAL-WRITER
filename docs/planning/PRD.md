...
**작성일**: 2026-03-22
**상태**: 승인됨
**변경 이력**: v1.0(2026-03-17) → v2.0(2026-03-18) 수주 최적화 시나리오 12건 반영 → v3.0(2026-03-20) 대화형 AI 파이프라인 추가 → v3.1(2026-03-21) 기능 상태 현행화 + API 키 DB 암호화 반영 → v3.2(2026-03-22) F-158~F-164 벡터 경량화 + RAG 질의응답 + 이미지 하이브리드 + 프롬프트 고도화
...
RFP(제안요청서)를 업로드하면 AI가 **수주 최적화 관점의 7단계 분석**, 전략 수립, 목차 구성, 내용 생성, **자동 검증**, **가격 제안서**, 최종 산출물(Word/PPT) 출력까지 전 과정을 자동화하는 **멀티 LLM 기반** 제안서 작성 솔루션. v3.0에서는 각 단계에서 **대화형 AI 코칭**(Multi-turn 질의응답)을 통해 사용자와 AI가 협력하여 더 정교한 결과를 생성한다.
...
- **온프레미스/클라우드 하이브리드**: Docker Compose 기반 유연한 배포
...
- 텍스트 추출 및 구조화 (@opendataloader/pdf로 표 구조 보존)
- 업로드 완료 시 드롭존 비활성화 + 파일 정보 표시
- 파일 삭제 기능 (벡터 데이터 생성 전에만 허용)
- 벡터 생성 시작 후 재업로드/삭제 불가
...
| F-004 | 방향성 제시 | AI가 3~5개 제안 방향 후보 생성 | ✅ 구현됨 |
| F-005 | 방향성 선택 | 사용자가 방향을 선택/확정 | ✅ 구현됨 |
| F-006 | 전략 생성 | 경쟁 전략, 차별화 포인트 자동 도출 | ✅ 구현됨 |
| F-007 | 목차 자동 구성 | 공공 제안서 표준 기반 목차 생성 | ✅ 구현됨 |
| F-008 | 목차 편집 | 목차 항목 추가/삭제/순서 변경 | ✅ 구현됨 |
| F-009 | 섹션별 내용 생성 | 각 목차 항목에 대한 상세 내용 AI 생성 | ✅ 구현됨 |
| F-010 | 내용 편집 | 생성된 내용 실시간 수정 | ✅ 구현됨 |
| F-011 | Word 출력 | 완성된 제안서를 .docx 파일로 다운로드 | ✅ 구현됨 |
| F-012 | PPT 출력 | 발표용 요약 장표를 .pptx 파일로 다운로드 | ✅ 구현됨 |
| F-013 | 프로젝트 관리 | 프로젝트 생성/목록/삭제, 진행 상태 관리 | ✅ 구현됨 |
| F-014 | 멀티 LLM 지원 | GPT + Claude 선택/전환 지원 | ✅ 구현됨 |
| F-015 | 커스텀 JWT 인증 | 이메일/비밀번호 + 커스텀 JWT (jose + bcryptjs) + 세션 관리 | ✅ 구현됨 |
| F-016 | 제안서 작성 가이드 | 팁 패널 + 가이드 페이지 + AI 코칭 | ✅ 구현됨 |
| F-017 | 제안서 자동 검증 리포트 | proposal-reviewer 에이전트, 예상 점수, 충족도, 개선사항 | ✅ 구현됨 |
| F-018 | 섹션 편집기 + 요구사항 추적 패널 | REQ-ID 커버리지 실시간 표시 | ✅ 구현됨 |
| F-019 | 단계별 검증 게이트 | 각 단계 완료 시 자동 품질 체크 | ✅ 구현됨 |
...
| F-112 | 프롬프트 관리 시스템 | 9개 LLM 프롬프트를 기능/내용/용도로 정의, DB 기반 CRUD, 버전 이력, 테스트 실행, 관리자 UI |
| F-113 | 제안서 관리 대시보드 | 프로젝트 목록에 RFP 요약/담당자/진행률 표시, 필터/검색/테이블 뷰 |
| F-114 | 담당자 관리 | 프로젝트별 담당자 배정/역할변경/제거, proposal_pm 이상 권한 |
| F-115 | RFP 파일 DB 저장 | 파일시스템 대신 PostgreSQL bytea에 직접 저장, 다운로드 API 제공 |
| F-120 | 대화형 AI 코칭 | 각 단계에서 Multi-turn 질의응답, 대화 이력 DB 저장, 프로젝트 컨텍스트 자동 주입 |
| F-121 | LLM 호출 비용/토큰 추적 | 모든 LLM 호출의 토큰 사용량, 비용, 지연시간 기록 + 관리자 대시보드 |
| F-122 | AI 채팅 패널 UI | 각 단계 페이지 우측 슬라이드 패널, SSE 스트리밍, 마크다운 렌더링 |
| F-130 | RFP 7단계 체이닝 분석 | 1회 호출 → 7단계 분리, 각 단계 결과 즉시 DB 저장, 이전 결과를 컨텍스트로 누적 |
| F-131 | GPT 파일 업로드 분석 | PDF를 GPT file_search로 직접 업로드, 표/이미지/도식 분석 포함 |
| F-132 | 분석 중단/재시작 | 각 단계 상태 DB 저장, 중단 후 마지막 완료 단계에서 재개 |
| F-133 | 단계별 사용자 개입 | 각 단계 결과 확인 → [확인/재실행/편집] 선택, 프롬프트 수정 후 재실행 |
| F-134 | 프롬프트 자동 생성 | 단계에 프롬프트 없으면 메타 프롬프트로 GPT에 프롬프트 생성 요청 → DB 저장 → 실행 |
| F-135 | 대용량 RFP 청크 분할 | 장/절 패턴 기반 분할, 각 청크에 이전 요약 컨텍스트 포함 |
| F-140 | API 키 DB 관리 | AI API 키를 AES-256-GCM 암호화하여 DB 저장, 설정 페이지에서 관리, 마스킹 표시 |
| F-150 | Qdrant 벡터 DB | 로컬 Qdrant로 텍스트 벡터 + 이미지 메타데이터 관리, Docker Compose 통합 |
| F-151 | RAG 기반 RFP 분석 | 프롬프트에 rawText 포함하지 않고 Qdrant RAG 검색으로 관련 청크만 전달 |
| F-152 | Vision 하이브리드 분석 | PDF 페이지별 이미지 변환 + GPT-5.4 Vision으로 표/도표/이미지 분석 |
| F-153 | RAG 청킹 최적화 | 4096토큰 기반 청킹 + 간이 토큰 추정, Top-15 검색, 이미지 5장 |
| F-154 | GPT-5.x 모델 지원 | GPT-5.4/5.4-mini/5.4-pro 모델 추가, 모델 목록 중앙 관리 (lib/ai/models.ts) |
| F-155 | 벡터 데이터 생성 단계 분리 | 벡터 생성을 독립 단계로 분리 (11단계 사이드바), SSE 실시간 진행률, vectorized 상태 추가 |
| F-156 | PDF 마크다운 변환 | @opendataloader/pdf로 표 구조를 마크다운으로 보존, pdf-parse 교체, 청크 2048토큰 |
| F-157 | 이미지 메타데이터 벡터화 | 이미지 페이지 선택적 PNG 변환 + GPT Vision 설명+키워드 생성 → Qdrant 벡터화, RAG 검색 시 이미지 매칭 |
| F-158 | 벡터 파이프라인 경량화 | pdf2pic 제거, Vision on-demand 전환, 9단계 파이프라인 |
| F-159 | RAG 기반 RFP 질의응답 | AI 코칭 → RAG 검색 + on-demand Vision 챗봇, DB 대화 저장 제거, react-markdown 렌더링 |
| F-160 | RAG 검색 성능 개선 | 구조 인식 청킹(1024토큰), Step별 searchQuery, score_threshold 0.4, 임베딩 안전 분할 |
| F-161 | 하이브리드 이미지 추출 | PyMuPDF get_images + get_pixmap 하이브리드, LLM Vision 필터링(keep/skip) |
| F-162 | API 키 에러 핸들링 | ensureProviderFromDb 매번 DB 조회, validateApiKey 사전검증, AI_KEY_ERROR 안내 |
| F-163 | 벡터 데이터 뷰어 | 텍스트 청크/이미지 벡터 View 버튼으로 Dialog에서 데이터 확인 |
| F-164 | 7단계 프롬프트 고도화 | 범용 프롬프트 17개 필드, Step별 searchQuery, DB seed 관리, Step 1 rawText 직접 전달 |
| F-165 | 공통 로딩 모달 | dim 배경 + stepper + progress bar 모달 (7개 페이지 적용) — 미구현 |
...
| 항목 | 요구사항 |
| --- | --- |
| 인증 | 커스텀 JWT 인증 (jose + bcryptjs) |
| 데이터 저장 | PostgreSQL 16 (Docker) + Drizzle ORM |
| API 통신 | LLM API 호출 시 TLS 1.3 암호화 |
| 파일 접근 | 로컬 파일 시스템 + RBAC 기반 접근 제어 |
| API 키 관리 | DB 저장 (AES-256-GCM 암호화) + 환경변수 폴백, UI 마스킹 표시 |
| 감사 로그 | 주요 작업 로그 기록 |
...
interface Project {
  id: string;            // UUID
  userId: string;        // 사용자 ID (profiles.id)
  title: string;
...
// v3.0 신규: 분석 단계 결과 (F-130)
interface AnalysisStep {
  id: string;
  projectId: string;
  stepNumber: number;        // 1~7
  slug: string;              // 'rfp-step1-overview' 등
  status: 'pending' | 'running' | 'completed' | 'failed';
  result: Record<string, unknown> | null;  // 단계별 JSON 결과
  promptUsed: string | null; // 실제 사용된 프롬프트 (디버깅/재현용)
...
interface Conversation {
  ...
  projectId: string;
  userId: string;
  topic: string;              // 'rfp-analysis', 'strategy-coaching', 'section-editing'
  stageContext: Record<string, unknown>;  // 현재 파이프라인 단계 컨텍스트
  status: 'active' | 'archived';
  createdAt: Date;
  lastMessageAt: Date;
}

// v3.0 신규: 메시지 이력 (F-120)
interface Message {
  id: string;
...
// v3.0 신규: LLM 호출 로그 (F-121)
interface LlmCallLog {
  id: string;
  projectId: string;
  conversationId: string | null;
  service: string;            // 'analysis', 'direction', 'strategy' 등
  provider: 'claude' | 'gpt';
  model: string;
  promptTokens: number;
...
// v2.1 신규: 프롬프트 템플릿 (F-112)
type PromptCategory = 'analysis' | 'generation' | 'review' | 'coaching' | 'price';

interface PromptTemplate {
  id: string;
  slug: string;              // 'rfp-analysis', 'direction-generation' 등
  name: string;              // 'RFP 분석'
  description: string;       // 기능·용도 설명
  category: PromptCategory;
  systemPrompt: string;      // System Prompt 전문
  userPromptTemplate: string; // {{variable}} 플레이스홀더 포함 템플릿
  maxTokens: number;
  version: number;
  isActive: boolean;
...
interface PromptTemplateVersion {
  id: string;
  templateId: string;        // FK → PromptTemplate
  version: number;
  systemPrompt: string;
  userPromptTemplate: string;
  maxTokens: number;
  changedBy: string;         // FK → Profile
  changeNote: string;
  createdAt: Date;
}
...
- **메타데이터**: PostgreSQL 16 (Docker) + Drizzle ORM
- **파일 저장**: 로컬 파일 시스템 (data/uploads, data/outputs, data/templates)
...
| PUT | /api/projects/:id/rfp/analyze/step/:stepNum | 단계 결과 직접 수정 |
...
| AI 채팅 패널 | (슬라이드 패널) | 각 단계 페이지 우측에서 AI와 대화, 스트리밍 응답 |
| LLM 사용량 | /admin/llm-logs | 토큰 사용량, 비용 요약, 프로바이더별 통계 |
...
- **AiChatPanel**: 우측 슬라이드 대화 패널 (SSE 스트리밍, 마크다운, 이력 관리)
- **LlmUsageDashboard**: LLM 호출 비용/토큰 사용량 대시보드
...
|                인프라 레이어                          |
|  +----------+ +----------+ +-------------------+  |
|  | PostgreSQL | | 로컬    | | 멀티 LLM          |  |
|  | 16 Docker | | 파일    | | (Claude + GPT)    |  |
|  | (Drizzle) | | 시스템  | |                   |  |
|  +----------+ +----------+ +-------------------+  |
|  +----------+ +----------+                        |
|  | 커스텀  | | Stage    |                        |
...
| DB | PostgreSQL 16 + Drizzle ORM (Docker) | 자체 호스팅, RBAC, 멀티유저 |
| 인증 | 커스텀 JWT (jose + bcryptjs) | 이메일/PW 인증 |
| 파일 저장 | 로컬 파일 시스템 | RBAC 기반 접근 제어 |
...
**포함**: PostgreSQL (Docker), 커스텀 JWT 인증, 멀티 LLM (GPT + Claude), RFP 업로드, 7단계 분석, 가이드 페이지
...
| FastAPI 별도 서버 불필요 | v3.0 아키텍처 결정 | Next.js + DB 기반 대화 이력으로 대화형 AI 구현 충분. Docker 자체 호스팅으로 타임아웃 제한 없음. 향후 벡터DB/RAG 필요 시 FastAPI 사이드카 추가 가능 |
