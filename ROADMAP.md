...
RFP를 업로드하면 AI가 분석하여 완성된 제안서(Word/PPT)를 자동 생성하는 온프레미스 솔루션

**📅 최종 업데이트**: 2026-03-18
**📊 진행 상황**: 8/34 Tasks 완료 (24%)
...
- **RFP 분석**: PDF/DOCX 업로드 → AI가 요구사항, 평가기준 자동 추출
- **전략 수립**: 방향성 제시 → 경쟁 전략 → 목차 자동 구성
- **문서 생성**: 섹션별 AI 내용 생성 → Word/PPT 자동 출력
...
- **Task 023: PostgreSQL 전환** - 우선순위
  - See: `/tasks/023-postgresql-migration.md`
  - SQLite → PostgreSQL, Drizzle ORM 드라이버 교체
  - drizzle-kit 마이그레이션 도입

- **Task 024: 인증 시스템** - 우선순위
  - See: `/tasks/024-auth-system.md`
  - 자체 ID/PW + 세션 기반 인증
  - 로그인 페이지, 미들웨어, useAuth Hook
...
- **Task 009: RFP 분석 기능** - 우선순위
  - See: `/tasks/009-rfp-analysis.md`
  - pdf-parse v2 호환성 수정 필요 (빌드 이슈)
...
| 주차 | 태스크 | 설명 |
| --- | --- | --- |
| 1-2 | 023 | PostgreSQL 전환 |
| 3-4 | 024 + 026 | 인증 + 에러/로깅 |
| 5 | 025 + 028 | RBAC + 파일 보안 |
| 6 | 027 + 029 | API 보안 + 테스트 |
| 7-8 | 009-011 | RFP 분석 + 방향 + 전략 |
| 9-10 | 012, 020-022 | 목차 관련 |
| 11-12 | 013-015 | 내용 생성 + 문서 렌더링 |
| 13 | 016-018 | 출력/템플릿/다이어그램 |
| 14-15 | 030-031 | 관리자 + 감사 로그 |
| 16 | 032 | Docker 배포 |
| 17 | 033-034 | 백업 + 커스터마이징 |
| 18 | 035-036 | SSO + 운영 문서 |
