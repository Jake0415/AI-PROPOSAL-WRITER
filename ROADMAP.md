...
- **Task 056: RFP 분석 파이프라인 고도화**
  ...
  - 7단계 체이닝 분석 (1회→7회 LLM 호출, 단계별 결과 DB 저장)
  - GPT 파일 업로드 (file_search로 표/이미지/도식 분석)
  - 중단/재시작 (마지막 완료 단계에서 재개)
  - 사용자 개입 (결과 확인→수정/재실행/편집)
  - 프롬프트 자동 생성 (메타 프롬프트로 GPT에 생성 요청→DB 저장)
  - 대용량 RFP 청크 분할 (장/절 패턴 기반)
