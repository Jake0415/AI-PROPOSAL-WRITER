...
- **Task 028: 파일 업로드 보안 강화**
  ...
  - (일부 구현됨: `lib/security/sanitize.ts`)
...
| --- | ---------- | --------------------------------- |
| 025 | RBAC       | 역할 기반 접근제어                |
| 026 | 에러/로깅  | 구조화 에러 처리 + pino 로거      |
| 027 | API 보안   | CSRF, Rate Limiting, CSP          |
| 028 | 파일 보안  | 업로드 보안 강화 (일부 구현)      |
| 029 | 테스트     | Vitest + Playwright (현재 ~5%)    |

### 단기 (기능 완성) - 4개

| #   | 제목           | 설명                              |
| --- | -------------- | --------------------------------- |
| 017 | 템플릿 관리    | docxtemplater 연동                |
| 021 | 업종별 템플릿  | SI/컨설팅/유지보수 기본 템플릿    |
| 022 | 평가항목 매핑  | 배점 비례 페이지 자동 배분        |
| 044 | 버전 관리      | 스냅샷/diff/복원                  |
...
| 4-5 | 017, 021-022 | 템플릿 + 매핑 |
| 6 | 044 | 버전 관리 |
| 7-8 | 030-031 | 관리자 + 감사 로그 |
| 9 | 032 | Docker 배포 |
| 10 | 033-034 | 백업 + 커스터마이징 |
| 11 | 035-036 | SSO + 운영 문서 |
