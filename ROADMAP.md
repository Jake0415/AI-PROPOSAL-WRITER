...
- **Task 055: RFP 업로드 UX 개선** ✅ - 완료
  - See: `/tasks/055-upload-ux-improvement.md`
  - Progress 인라인 통합, 업로드 중 드래그 영역 숨김, 버튼명 변경
...
- **Task 022: 평가항목-목차 매핑** (진행중)
  - See: `/tasks/022-evaluation-mapping.md`
  - RFP 평가항목 1:1 대응, 배점 비례 페이지 배분 (UI 컴포넌트 일부 구현)
...
- **Task 039: 섹션 편집기 + 요구사항 추적 패널** ✅ - 완료
  ...
  - `linkedReqIds` DB 컬럼 추가, `ReqTrackingPanel` 컴포넌트 (카테고리별 커버리지, 미반영 경고)
...
- **Task 044: 제안서 버전 관리 + 비교** ✅ - 완료
  - See: `/tasks/044-version-management.md`
  - 스냅샷 저장, 버전 비교, 타임라인 UI
...
- **Task 017: 사용자 템플릿 관리** (진행중)
  - See: `/tasks/017-template-upload.md`
  - 업로드/삭제 API + UI 구현, docxtemplater 연동 미완
...
- **Task 031: 감사 로그 시스템** (진행중)
  - See: `/tasks/031-audit-logs.md`
  - 테이블 + API 구현, 관리자 뷰어 미완
...
- **Task 033: 백업/복구 시스템** (진행중)
  - See: `/tasks/033-backup-restore.md`
  - export/import API 구현, 자동 스케줄 미완

- **Task 034: 회사별 커스터마이징** (진행중)
  - See: `/tasks/034-company-customization.md`
  - 테넌트 설정 API 구현, 동적 로딩 미완

- **Task 036: 운영 문서** (진행중)
  - See: `/tasks/036-operations-docs.md`
  - docs/ 파일 존재, 상세 내용 보완 필요

- **Task 045: 프롬프트 관리 시스템** ✅ - 완료
  - See: `/tasks/045-prompt-management.md`
  - DB 기반 CRUD + 버전 관리 + 관리자 UI

- **Task 057: API 키 DB 관리** ✅ - 완료
  - See: `/tasks/057-api-key-db-management.md`
  - AES-256-GCM 암호화 저장, 설정 페이지에서 관리, 마스킹 표시
...
- **Task 051: RFP 파일 저장 방식 결정** ✅ - 완료
  - See: `/tasks/051-rfp-file-db-storage.md`
  - bytea 기각 → 파일시스템(data/uploads/) 채택, 다운로드 API 구현

### Phase 8: 대화형 AI 파이프라인

- **Task 052: 대화 DB 스키마 확장** ✅ - 완료
  - See: `/tasks/052-conversation-db.md`
  - conversations, messages, llm_call_logs 테이블 + Repository

- **Task 053: 대화형 AI 서비스 + API** ✅ - 완료
  - See: `/tasks/053-conversation-service.md`
  - conversation.service + 대화 API 5개 + LLM 로그 API

- **Task 054: AI 채팅 패널 UI** ✅ - 완료
  - See: `/tasks/054-ai-chat-panel.md`
  - 우측 슬라이드 패널, 5개 단계 페이지에 채팅 버튼

### Phase 9: RFP 분석 고도화 ✅

- **Task 056: RFP 분석 파이프라인 고도화** ✅ - 완료
  - See: `/tasks/056-rfp-analysis-pipeline.md`
  - 7단계 프롬프트 + DB + GPT 파일 업로드 + 청크 분할 + API + UI

- **Task 058: Qdrant 기반 RAG + Vision 하이브리드** ✅ - 완료
  - See: `/tasks/058-qdrant-rag-vision.md`
  - 로컬 Qdrant 벡터 DB + OpenAI Embeddings
  - PDF 페이지별 이미지 변환 + GPT-4o Vision
  - RAG 검색으로 컨텍스트 초과 해결

- **Task 059: RAG 청킹 최적화 + GPT-5.x 모델 지원** ✅ - 완료
  - See: `/tasks/059-chunking-model-upgrade.md`
  - 청킹 500자 → 4096토큰 업그레이드 (간이 토큰 추정)
  - GPT-5.4/5.4-mini/5.4-pro 모델 추가, 모델 목록 중앙 관리
  - RAG topK 15, 이미지 5장으로 확대

- **Task 060: 벡터 데이터 생성 단계 분리** ✅ - 완료
  - See: `/tasks/060-vector-step-separation.md`
  - 벡터 생성을 독립 단계로 분리 (11단계 사이드바)
  - SSE 스트리밍 실시간 진행률 (6단계 스테퍼 + 배치별)
  - ProjectStatus에 vectorized 상태 추가

- **Task 061: PDF 마크다운 변환 + 이미지 메타데이터 + GPT-5.x 호환** ✅ - 완료
  - See: `/tasks/061-pdf-markdown-image-meta.md`
  - @opendataloader/pdf로 표 구조 마크다운 보존
  - 청크 4096→2048토큰 축소
  - 이미지 페이지 선택적 변환 + GPT Vision 메타데이터 생성
  - 이미지 설명+키워드 벡터화 → RAG 검색 시 이미지 매칭
  - GPT-5.x max_tokens→max_completion_tokens API 호환 처리
  - "분석 결과" → "RFP 분석" 라벨 변경 + 벡터 완료 후 다음 단계 버튼

- **Task 062: 벡터 파이프라인 경량화** ✅ - 완료
  - See: `/tasks/062-vector-pipeline-lightweight.md`
  - pdf2pic 페이지 PNG 변환 제거 + PyMuPDF 개별 이미지만 추출
  - 벡터 생성 시 Vision 호출 제거 → 분석 시 on-demand Vision
  - 10단계 → 7단계 경량화

- **Task 063: RAG 기반 RFP 질의응답 챗봇** ✅ - 완료
  - See: `/tasks/063-rfp-ask-chatbot.md`
  - AI 코칭 → RAG 기반 자유 질의응답으로 전환
  - DB 대화 저장 제거 → 세션 기반 로컬 state
  - RAG 검색 + on-demand Vision + SSE 스트리밍

- **Task 065: 하이브리드 이미지 추출 + Vision 필터링** ✅ - 완료
  - See: `/tasks/065-hybrid-image-extraction.md`
  - PyMuPDF 이미지 객체 + imagePages 페이지 렌더링 하이브리드
  - LLM Vision 필터링 (keep/skip + description 동시 생성)
  - 9단계 벡터 파이프라인

- **Task 064: RAG 검색 성능 개선** ✅ - 완료
  - See: `/tasks/064-rag-performance.md`
  - 구조 인식 청킹 (마크다운 섹션 + 테이블 보존)
  - 청크 1024토큰, 토큰 추정 1.3, Step 검색 쿼리 구체화
  - Qdrant score_threshold 0.4 필터링

- **Task 071: 상단 메뉴 재구성 + 설정 카드 대시보드** (진행중)
  - See: `/tasks/071-menu-settings-dashboard.md`
  - 상단 메뉴: 제안서 작성 / 프롬프트 관리 / 템플릿 관리 / 사용 가이드 / 설정
  - 설정 페이지: 7개 카드 대시보드 (운용현황, AI LLM, 사용자, 브랜딩, 데이터, 감사로그, 버전)
  - admin 6개 개별 링크 → 설정 카드로 통합

- **Task 070: 목차 구성 페이지 재설계** ✅ - 완료
  - See: `/tasks/070-outline-page-redesign.md`
  - level 0-indexed 통일 + evalId/estimatedPages 연결
  - 트리 에디터 (추가/수정/삭제/이동/인라인 편집)
  - 배점 기반 페이지 자동 배분 + 번호 동적 계산
  - AI/템플릿 탭 전환 + totalPages DB 컬럼

- **Task 069: 내용 생성 페이지 재설계** ✅ - 완료
  - See: `/tasks/069-section-page-redesign.md`
  - 대분류 탭 + 챕터별 단계적 AI 생성 + RAG 연동
  - Pretty Print / 마크다운 편집 / JSON 편집 3탭
  - 우측 작성 팁 패널 (StepTipsPanel)

- **Task 068: 데이터 로딩 UX 개선** ✅ - 완료
  - 방향성/전략/목차 페이지 진입 시 DataLoadingSpinner 표시
  - catch에서 자동 생성 방지 (세션 만료 시 의도치 않은 재생성 차단)

- **Task 066: API Key 401 Error Handling & Seed Resilience** ✅ - 완료
  - See: `/tasks/066-api-key-error-handling.md`
  - seed-data.ts AI 설정을 AI_PROVIDER 환경변수 기반으로 복원
  - generateText/generateStream에 API 키 사전검증 추가
  - 401/AI_KEY_ERROR 에러 코드 구분 + 설정 페이지 안내
  - useSSE에 errorCode 필드 추가

- **Task 067: Global Loading Modal Component** (TODO)
  - See: `/tasks/067-global-loading-modal.md`
  - 공통 LoadingModal 컴포넌트 (dim 배경 + 단계별 stepper + progress bar)
  - LoadingModalProvider (Context) + useLoadingModal 훅
  - 7개 페이지 적용 (analysis, vectorize, direction, strategy, sections, pricing, review)
  - 에러 상태 시각화 + 설정 링크 액션 버튼
...
| 순서 | #   | 제목                    | 규모 | 설명                                          |
| ---- | --- | ----------------------- | ---- | --------------------------------------------- |
| 0-1  | 067 | 공통 로딩 모달          | L    | dim + stepper + progress 모달 (7개 페이지 적용) |
...
| 5    | 031 | 감사 로그           | M    | audit_logs 관리자 뷰어 + 미들웨어   |

### Phase 3: 백업 + 문서

| 순서 | #   | 제목      | 규모 | 설명                            |
| ---- | --- | --------- | ---- | ------------------------------- |
| 6    | 033 | 백업/복구 | S    | JSON 내보내기/가져오기 + pg_dump |
| 7    | 036 | 운영 문서 | S    | 설치/운영/API/사용자 매뉴얼     |
