...
export const rfpImageMetadata = aiprowriterSchema.table('rfp_image_metadata', {
  ...
  imageType: text('image_type').$type<'element' | 'page_render'>().notNull(),
  imagePath: text('image_path').notNull(),
  width: integer('width'),
  height: integer('height'),
  description: text('description'),
  keywords: jsonb('keywords').$type<string[]>().default([]),
  filterStatus: text('filter_status').$type<'keep' | 'skip' | 'auto_skip' | 'pending'>().notNull().default('pending'),
...
export const proposalSections = aiprowriterSchema.table('proposal_sections', {
  id: uuid('id').defaultRandom().primaryKey(),
  projectId: uuid('project_id').notNull().references(() => projects.id, { onDelete: 'cascade' }),
  outlineId: uuid('outline_id').notNull().references(() => proposalOutlines.id, { onDelete: 'cascade' }),
